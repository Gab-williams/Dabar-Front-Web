...
export default function BlogDetails() {
  ...
  useEffect(()=>{
    ...
    await  apiClient.get('/sanctum/csrf-cookie')
      ...
      if (selectedx === 'GB') {
        const translatedData = randomstory.data.success.map((item) => {
  ...
  //      else {
  //       // console.log('other');
  //       body =   <div dangerouslySetInnerHTML={{ __html: fieldsdata.body }} />
  //       //<Markup html={fieldsdata.body} />;
  //       console.log(body)
  //     }
  //   } catch (error) {
...
// console.log(stringWithoutStyle)
  //  return <Markup html={stringWithoutStyle} />
  //  return parse(stringWithoutStyle)
   ...
   return parse(stringWithoutStyle, {
    replace: (node) => {
      if (node.name === 'a' && node.attribs && node.attribs.href) {
        ...
        return (
          <Link href={node.attribs.href} target={node.attribs.target} style={{ color:'#fd9005' }}>
     ...
     // const nonEmptyNodes = Array.from(cont.childNodes).filter(node =>  node.textContent.trim() !== '');
      // console.log( titler.childNodes)
        ...
        if (selectedx === 'GB') {
         ...
         let ansdateandtime =  await changlang("EN", dateandtime.textContent)
          ...
          titlexs.textContent = fieldsdata.heading
          dateandtime.textContent = formattedDate
          readtimexs.textContent = readtime
      ...
      for (var i = 0; i < cont.childNodes.length; i++) {
        if (selectedx === 'GB') {
          ...
          var styleRegex = /style\s*=\s*"([^"]*)"/g;
          var stringWithoutStyle = fieldsdata.body.replace(styleRegex, '');
        let htmlparseans =   parse(stringWithoutStyle, {
            replace: (node) => {
              if (node.name === 'a' && node.attribs && node.attribs.href) {
                  // console.log(node.attribs.href, )
                return (
         ...
         cont.innerHtml =  htmlparseans       
         }else
          if (selectedx !== 'GB' && cont.childNodes[i].innerText.trim() != "" ) {
