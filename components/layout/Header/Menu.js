...
export default function Menu({ handleMobileMenuOpen, handleSidebarOpen, offCanvasNav, logoAlt, white }) {
  ...
  const {selectedx, setSelectedx} = created
    ...
    const moretry = useRef(null)
      ...
      const handleLinkPage = (name)=>{
      //  window.location.href = `${urllink}/business?hello=${encodeURIComponent(name)}`
      router.replace(`/business?hello=${encodeURIComponent(name)}`)
      }
    return (
        <>
                    ...
                    <div className="tgmenu__navbar-wrap tgmenu__main-menu d-none d-lg-flex">
                        <ul className="navigation">
                            {categoryMenux?.slice(0,3).map((item, i)=>{
                              if(hello){
                                return <li key={i} style={{ cursor:"pointer" }} onClick={()=>handleLinkPage(item.name)} className={hello.toLowerCase() == item.name.toLowerCase() ? "active" : ""}><a  onClick={()=>handleLinkPage(item.name)} >{item.title}</a></li>

                              }else{
                                return <li key={i} style={{ cursor:"pointer" }}  onClick={()=>handleLinkPage(item.name)} className={router.pathname.toLowerCase() == item.name.toLowerCase() ? "active" : ""}><a onClick={()=>handleLinkPage(item.name)}>{item.title}</a></li>
                        ...
                        {/* <li className={router.pathname == "/business" ? "active" : ""}><Link href="/business?hello=stephen">Business Insights</Link></li>
                            ...
                            <li className="menu-item-has-children"><Link href="#" ref={moretry} className='morex'>More</Link>
                                ...
                                {categoryMenux?.slice(3,8).map((item, i)=>{
                              if(hello){
                                return <li key={i} style={{ cursor:"pointer" }}  onClick={()=>handleLinkPage(item.name)} className={hello.toLowerCase() == item.name.toLowerCase() ? "active" : ""}><a onClick={()=>handleLinkPage(item.name)}>{item.title}</a></li>
